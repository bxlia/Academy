...
Human** LoadFromFile(const std::string& filename, int& count);
...
void main()
{
	...
	int loaded_count = 0;
	Human** loaded_group = LoadFromFile("group.txt", loaded_count);

	if (loaded_group && loaded_count > 0)
	{
		Print(loaded_group, loaded_count);

		for (int i = 0; i < loaded_count; i++)
		{
			delete loaded_group[i];
		}
		delete[] loaded_group;
	}
...
Human** LoadFromFile(const std::string& filename, int& count)
{
	...
	{
		std::string type, last_name, first_name, speciality, group_name, subject;
		time_t timestamp;
		...
		if (type == "Student:")
		{
			fin >> last_name >> first_name >> timestamp >> speciality >> group_name >> rating >> attendance;
			time_t temp_time = timestamp;
			tm* temp_tm = localtime(&temp_time);
			char date_str[11];
			strftime(date_str, sizeof(date_str), "%Y.%m.%d", temp_tm);

			group[i] = new Student(last_name, first_name, date_str, speciality, group_name, rating, attendance);
		}
		else if (type == "Teacher:")
		{
			fin >> last_name >> first_name >> timestamp >> speciality >> experience;
			time_t temp_time = timestamp;
			tm* temp_tm = localtime(&temp_time);
			char date_str[11];
			strftime(date_str, sizeof(date_str), "%Y.%m.%d", temp_tm);

			group[i] = new Teacher(last_name, first_name, date_str, speciality, experience);
		}
		else if (type == "Graduate:")
		{
			...
			group[i] = new Graduate(last_name, first_name, date_str, speciality, group_name, rating, attendance, subject);
		}
		else
		{
